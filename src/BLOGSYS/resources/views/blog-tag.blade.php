	...
	<x-slot name="seo_config">
        ...
        <section class="p-4 py-24 min-h-screen" id="header">
            <div class="max-w-4xl mx-auto px-5 text-center">
                <h1 class="text-4xl font-bold uppercase text-sky-500 mt-10">Blog Tag</h1>
                <div>
@foreach($tags as $key => $tag)
                    <a href="{{ route('tag.detail',$key) }}" class="text-sky-800 hover:text-sky-500 mx-4 mt-8 inline-block lowercase">{{ $key }} <span class="bg-sky-700 text-white px-2 rounded-full">{{ $tag }}</span></a>
@endforeach
                </div>
            </div>
        </section>
